...
os.environ['REPLICATE_API_TOKEN']='r8_SRNkGRGRuQji9YzCmopRNfaJ0QyMCrr1syLtv'
os.environ['OPENAI_API_KEY']='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'
...
def get_mixtral_score(query, responses):
    if Testing:
        return 'Mixtral'
    input = {
        "top_p": 1,
        ...
        "temperature": 0.5,
        "system_prompt": f"You a smart assistant that is asked to evaluate several response to the following question:{query}.\n*please replay only with the id of the best response formated as {id}",
        "max_new_tokens": 5
    }
    out = ''
    for chunk in replicate.stream("mistralai/mistral-7b-instruct-v0.2",input=input):
        out +=str(chunk)
    print(out)
    out = out.replace('id:', '').replace('\n', ' ')
    for k in model_ids:
        out = out.replace(k, model_ids[k])
    return out 
...
def generate_all(user_prompt, context):
    ...
    yield get_mixtral_score(user_prompt, responses)
    yield '</h4> '
